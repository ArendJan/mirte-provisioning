...
class RosPackages(ProvisioningModule):
    def __init__(self) -> None:
        pass

    def start(self, mount_point: str, loop: AbstractEventLoop) -> None:
        loop.create_task(self.install_ros_packages(mount_point))

    def stop(self) -> None:
        print("stop ROS packages")

    async def install_ros_packages(self, mount_point: str) -> None:
        try:
            packages_folder = os.path.join(mount_point, "ros_packages")
            target_ws_folder = self.get_ws_folder()
